...
interface Props {
  ...
  onBuyCredits: () => void;
  // Optional handoff from the List Builder: pre-loaded records + mapping
  // that jump straight to the "map" step.
  initialRecords?: CSVRecord[] | null;
  initialColumnMap?: ColumnMap | null;
...
export default function NewTraceView({ session, credits, onTraceComplete, onBuyCredits, initialRecords, initialColumnMap, initialFileName }: Props) {
  ...
  // When the List Builder hands us a pulled list, load it straight into the map step.
  useEffect(() => {
    if (initialRecords && initialRecords.length > 0) {
      const hdrs = Object.keys(initialRecords[0]);
      setHeaders(hdrs);
      setFullData(initialRecords);
      setPreviewData(initialRecords.slice(0, 3));
      setColumnMap(initialColumnMap || EMPTY_MAP);
      setFileName(initialFileName || 'Pulled list');
      setStep('map');
    }
  ...
  const handleStartTrace = async () => {
    ...
    setError('');
    setProcessingPct(0);
    setProcessingMsg('Creating job...');

    const userId = session.user.id;

    try {
      // 1. Create the job (stores input server-side, returns an id).
      const createRes = await fetch('/api/skiptrace', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ records: fullData, columnMap, userId, fileName, userEmail: session.user.email }),
      });
      const created = await createRes.json();
      if (!createRes.ok) throw new Error(created.error || 'Could not start job');

      const jobId: string = created.jobId;
      const total: number = created.totalRecords || fullData.length;

      // 2. Drive the chunk loop, updating progress from real processed counts.
      let done = false;
      let hits = 0;
      let guard = 0;
      const maxIterations = Math.ceil(total / 1) + 50; // generous safety bound

      while (!done && guard++ < maxIterations) {
        ...
        if (data.status === 'failed') throw new Error('Job failed during processing');

        const processed = data.processedCount || 0;
        const pct = total > 0 ? Math.min(99, Math.round((processed / total) * 100)) : 0;
        setProcessingPct(data.done ? 100 : pct);
        setProcessingMsg(
          data.done ? 'Packaging your results...' : `Scanning records — ${processed.toLocaleString()} of ${total.toLocaleString()}`
        );
      ...
      if (!done) throw new Error('Processing did not complete — please check Job History.');

      // 3. Fetch the final results for download.
      const resultRes = await fetch(`/api/jobs/${jobId}?userId=${userId}`);
      const resultJson = await resultRes.json();
      if (!resultRes.ok) throw new Error(resultJson.error || 'Could not load results');

      const csvOut = Papa.unparse(resultJson.data);
      const blob = new Blob([csvOut], { type: 'text/csv;charset=utf-8;' });
      const url = URL.createObjectURL(blob);
      setDownloadUrl(url);
      setResult({ hits, total });
  ...
  const handleDownload = () => {
    ...
    link.download = `leadminer_${fileName || 'results'}_${Date.now()}.csv`;
    link.click();
  };
        ...
        <div className="space-y-6">
          <div className="rounded-2xl p-6 border" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-sm mt-0.5" style={{ color: 'var(--text-2)' }}>{fileName} · {fullData.length.toLocaleString()} records</p>
              </div>
          ...
          <div className="rounded-2xl p-6 border" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
            <div className="flex items-center justify-between mb-4">
              <div>
                <p className="text-sm" style={{ color: 'var(--text-2)' }}>Credits reserved (max)</p>
                <p className="text-3xl font-black" style={{ color: 'var(--navy)' }}>{fullData.length.toLocaleString()}</p>
                <p className="text-xs mt-1" style={{ color: '#059669' }}>✓ You're only charged for contacts we find</p>
              </div>
        ...
        <div className="rounded-2xl overflow-hidden border" style={{ background: 'var(--bg-surface)', borderColor: 'var(--border)' }}>
          ...
          <div className="px-8 py-5" style={{ background: 'var(--bg-surface)' }}>
            ...
            <p className="text-xs mt-2" style={{ color: 'var(--text-2)' }}>
              {fullData.length.toLocaleString()} records · processed in batches · you only pay for contacts found
            </p>
          </div>
        </div>
      )}
