...
// Real product images: paste a URL (e.g. an AliExpress/CJ product image such as
// https://ae01.alicdn.com/.../xxx.jpg) or a local path (/products/foo.jpg)
// against the product id. Anything listed here overrides the branded
// placeholder — no other code changes needed. Images are served unoptimized
// (next.config), so remote URLs work without extra configuration.
// Provide one URL or several (the first is the main image, the rest the gallery).
export const imageOverrides: Record<string, string[]> = {
  // "sk-vitc-serum": ["https://ae01.alicdn.com/kf/....jpg", "https://ae01.alicdn.com/kf/....jpg"],
};

// Fall back to self-hosted, on-brand placeholders (scripts/gen-images.mjs) so
// the store always renders even before real photos are added.
