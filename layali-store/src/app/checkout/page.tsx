...
const countries = ["السعودية", "الإمارات", "قطر", "الكويت", "البحرين", "عُمان", "مصر"];

export default function CheckoutPage() {
  const { items, subtotal, currency, clear, hydrated } = useStore();
  const [form, setForm] = useState({ name: "", phone: "", country: "السعودية", city: "", address: "", notes: "", payment: "cod" });
  ...
  const placeOrder = () => {
    if (!valid) return;
    const lines = items.map((i) => `• ${i.name} ×${i.qty} — ${formatPrice(i.price * i.qty, currency)}`).join("%0A");
    const msg =
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        ...
        <div className="space-y-4 lg:col-span-2">
          <div className="card p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="رقم الجوال (واتساب)" value={form.phone} onChange={(v) => set("phone", v)} placeholder="05XXXXXXXX" type="tel" />
              <div>
              ...
              <Field label="المدينة" value={form.city} onChange={(v) => set("city", v)} placeholder="مثال: الرياض" />
            </div>
            <div className="mt-4">
          ...
          <div className="card p-6">
            ...
            <label className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 ${form.payment === "card" ? "border-blush-500 bg-blush-50" : "border-plum-700/15"}`}>
              ...
              <span className="font-semibold text-plum-800">💳 بطاقة / Apple Pay / تابي — يرسل لكِ الفريق رابط دفع آمن</span>
            </label>
          </div>
        </div>
        ...
        <div className="card h-fit p-6">
          ...
          <div className="space-y-2">
            {items.map((i) => (
              <div key={i.id} className="flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-plum-900 ltr-nums">{formatPrice(i.price * i.qty, currency)}</span>
              </div>
            ))}
          </div>
          <div className="mt-4 flex items-center justify-between border-t border-plum-700/10 pt-3">
            ...
            <span className="font-bold text-blush-600 ltr-nums">{formatPrice(subtotal, currency)}</span>
          </div>
